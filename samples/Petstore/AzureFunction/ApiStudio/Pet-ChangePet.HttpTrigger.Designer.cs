namespace Pet
{
    ...
    {
        [Function(nameof(HttpChangePet.ChangePet))]
        [OpenApiOperation(operationId: "ChangePet", tags: new[] { "Pet" }, Summary = "ChangePet", Description = "This is a description of the rest verbs actions", Visibility = OpenApiVisibilityType.Important)]
		...
		[OpenApiRequestBody(contentType: "application/json", bodyType: typeof(Pet), Required = true, Description = "")]
		[OpenApiParameter(name: "PetId", In = ParameterLocation.Path, Required = true, Type = typeof(Guid), Summary = "A unique, immutable identifier that identifies a resource", Description = "A unique, immutable identifier that identifies a resource", Visibility = OpenApiVisibilityType.Important)]
		[OpenApiResponseWithBody(statusCode: HttpStatusCode.Accepted, contentType: "application/json", bodyType: typeof(Pet), Summary = "Accepted", Description = "Accepted", CustomHeaderType = typeof(ResponseHeaderOnSuccess))]
        ...
        public async Task<HttpResponseData> ChangePet(
            [HttpTrigger(AuthorizationLevel.Anonymous, "PUT", 
            Route = "pets/{PetId}")] HttpRequestData httpRequestData)
        {
