namespace ApiStudioIO.VsOptions.HttpSecurity
{
    ...
    class UITypeEditorDropDown : UITypeEditor
    {
        ...
        public override object EditValue(ITypeDescriptorContext context, IServiceProvider provider, object value)
        {
            ...
            foreach (var securityPattern in securityPatterns)
            {
                // we store benchmarks objects directly in the listbox
                int index = lb.Items.Add(securityPattern);
                if (securityPattern.Equals(value))
                {
        ...
        private readonly string[] securityPatterns =
            {
                "{Resource}::{Action}",
